...
const Chatbox = (props) => {
	...
	const userMsgUrl =
		"http://https://notfacebook-b2511391168d.herokuapp.com/user-message";
	const groupMsgUrl =
		"http://https://notfacebook-b2511391168d.herokuapp.com/group-message";
	...
	if (!props.grp) {
		...
		if (
			...
			followingCtx.following.includes(
				(following) => following.id === props.chatboxId
			)
		) {
			followingCtx.following.find(
				(followingUser) => followingUser.id === frdOrGrpId
			)["chat_noti"] = false;
			console.log("following (chatbox)", followingCtx.following);
		}
	} else {
	...
	useEffect(() => {
		...
		if (wsCtx.websocket !== null && wsCtx.newPrivateMsgsObj) {
			...
			if (wsCtx.newPrivateMsgsObj.sourceid === frdOrGrpId) {
				console.log(
					"new Received msg data when chatbox is open",
					wsCtx.newPrivateMsgsObj
				);
				console.log(
					"ws receives msg from when chatbox is open: ",
					wsCtx.newPrivateMsgsObj.sourceid
				);
				setNewMsgs((prevNewMsgs) => [
					...new Set([...prevNewMsgs, wsCtx.newPrivateMsgsObj]),
				]);
	...
	useEffect(() => {
		...
		if (wsCtx.websocket !== null && wsCtx.newGroupMsgsObj) {
			...
			if (wsCtx.newGroupMsgsObj.groupid === frdOrGrpId) {
				...
				console.log(
					"ws receives msg for group when chatbox is open: ",
					wsCtx.newGroupMsgsObj.groupid
				);
				setNewMsgs((prevNewMsgs) => [
					...new Set([...prevNewMsgs, wsCtx.newGroupMsgsObj]),
				]);
	...
	const sendMsgHandler = useCallback(
		...
		[
			...
			followingCtx.following,
			wsCtx.websocket,
			joinedGrpCtx.receiveMsgGroup,
			followingCtx.receiveMsgFollowing,
			props.chatboxId,
		]
	);
	...
	const fetchOldMsg = async (url) => {
		try {
			...
			if (data.data) {
				...
				if (props.grp) {
					// changing the prop name from sourceid to targetid if group
				...
				oldMsgArr.sort(
					(b, a) => Date.parse(b.createdat) - Date.parse(a.createdat)
				);
	...
	useEffect(() => {
		// fetch old pri msg
		!props.grp &&
			fetchOldMsg(`${userMsgUrl}?targetid=${selfId}&sourceid=${frdOrGrpId}`);
		// fetch old grp msg
	...
	return (
		<div className={styles["container"]}>
			<button onClick={closeChatboxHandler} className={styles["close-btn"]}>
				X
			</button>
			...
			<ChatboxMsgArea
				oldMsgItems={oldMsgData}
				newMsgItems={newMsgsData}
