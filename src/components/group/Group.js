...
function Group(props) {
	...
	useEffect(() => {
		...
		jGrpCtx.joinedGrps &&
			setCurrentlyJoined(
				jGrpCtx.joinedGrps.some((joinedGrp) => joinedGrp.id === +props.grpid)
			);
	...
	useEffect(() => {
		if (wsCtx.newNotiInvitationReplyObj) {
			if (wsCtx.newNotiInvitationReplyObj.accepted) {
				...
				const UserJoining = usersCtx.users.find(
					(user) => user.id === wsCtx.newNotiInvitationReplyObj.sourceid
				);
				const JoinGroup = grpCtx.groups.find(
					(group) => group.id === wsCtx.newNotiInvitationReplyObj.groupid
				);
				...
				console.log(
					"cur user has joined these groups (group)",
					jGrpCtx.joinedGrps
				);
		...
		<Card>
			<div className={classes.container}>
				...
				<div className={classes.btn}>
					{!currentlyJoined && !requestedToJoin && (
						<div
							className={classes.smallbtn}
							id={props.grpid}
							onClick={reqToJoinHandler}
						>
							Join
						</div>
					)}
					{!currentlyJoined && requestedToJoin && (
						<div className={classes.smallbtn} id={props.grpid}>
							Requested
						</div>
					)}
					{currentlyJoined && !requestedToJoin && (
