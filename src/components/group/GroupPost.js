...
function GroupPost(props) {
    ...
    const showCommentsHandler = (e) => {
        ...
        fetch(`hhttps://notfacebook-b2511391168d.herokuapp.com/group-post-comment?id=${e.target.id}`)
            .then(resp => resp.json())
            .then(data => {
    ...
    const createCommentHandler = (createCommentPayloadObj) => {
        ...
        fetch("hhttps://notfacebook-b2511391168d.herokuapp.com/group-post-comment", reqOptions)
            .then(resp => resp.json())
            .then(data => {
    ...
    const createCommentSuccessHandler = (createCommentSuccessful) => {
        ...
        fetch(`hhttps://notfacebook-b2511391168d.herokuapp.com/group-post-comment?id=${props.id}`)
            .then(resp => resp.json())
            .then(data => {
