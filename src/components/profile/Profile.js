...
function Profile({ userId }) {
	...
	useEffect(() => {
		followingData &&
			setIsFollower(
				followingData.some((follower) => follower.id == currUserId)
			);
	}, [followingData]);
	...
	useEffect(() => {
		if (wsCtx.newNotiFollowReplyObj) {
			if (wsCtx.newNotiFollowReplyObj.accepted) {
				...
				const followUser = usersCtx.users.find(
					(user) => user.id === wsCtx.newNotiFollowReplyObj.sourceid
				);
	...
	const unfollowHandler = (e) => {
		...
		const unfollowUser = usersCtx.users.find(
	...
	if (currUserId !== userId) {
		if (currentlyFollowing) {
			...
			followButton = (
				<div className={classes.followbtn} id={userId}>
					Requested
				</div>
			);
		} else {
			followButton = (
				<div className={classes.followbtn} id={userId} onClick={followHandler}>
					+ Follow
				</div>
			);
		}
						...
						<>
							...
							<div className={classes.column}>
								...
								<div className={classes.followers}>
									<div className={classes.follow} onClick={handleFollowerClick}>
