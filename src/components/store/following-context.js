...
export const FollowingContextProvider = (props) => {
			...
			.then((data) => {
				...
				if (!allChatItemArr || allChatItemArr.length === 0) {
					...
					usersCtx.users &&
						...
						!following &&
						setOtherListedChatUsers(
							usersCtx.users.filter((user) => user.public === 1)
						); // takes care if cur user is not following any user
					usersCtx.users &&
						usersCtx.users.length !== 0 &&
						following &&
						setOtherListedChatUsers(
							usersCtx.users.filter(
								(user) =>
									user.public === 1 &&
									!following.some(
				...
				if (allChatItemArr && allChatItemArr.length !== 0) {
					if (!following || following.length === 0) {
						...
						const allOtherChatItems = allChatItemArr.map((chatItem) => {
							const matchedOtherChatItem = usersCtx.users.find(
								(user) => user.id === chatItem.sourceid
							);
							return { ...chatItem, ...matchedOtherChatItem };
						});
						const allPublicUsers = usersCtx.users.filter(
							(user) => user.public === 1
						);
						// filter out all public users with chat record
						const filteredPublicUsers = allPublicUsers.filter(
							(user) =>
								!allOtherChatItems.some(
									(chatItem) => user.id === chatItem.sourceid
								)
						);
						setOtherListedChatUsers([
							...allOtherChatItems,
						...
						const filteredFollowingChatItems = allChatItemArr.filter(
							(chatItem) => {
								// console.log("chatItem.sourceid", chatItem.sourceid);
								if (!following) return false;
								return following.some(
									(followingUser) => followingUser.id === chatItem.sourceid
								);
							}
						);
						console.log(
							"filteredFollowingChatItems",
							filteredFollowingChatItems
						);

						// merge the properties for following WITH chat item
						const followingChatItems = filteredFollowingChatItems.map(
							(chatItem) => {
								console.log("chatItem.sourceid", chatItem.sourceid);
								const matchedFollowing = following.find(
									(followingUser) => followingUser.id === chatItem.sourceid
								);
						...
						const filteredFollowingNoChatItems = following.filter(
							(followingUser) => {
								return !allChatItemArr.some(
									(chatItem) => followingUser.id === chatItem.sourceid
								);
							}
						);
						console.log(
							"filteredFollowing Without oChatItems",
							filteredFollowingNoChatItems
						);

						let finalFollowingChatItems;
						if (
							followingChatItems &&
							followingChatItems.length !== 0 &&
							filteredFollowingNoChatItems &&
							filteredFollowingNoChatItems.length !== 0
						) {
							...
							(!followingChatItems || followingChatItems.length === 0) &&
							filteredFollowingNoChatItems &&
							filteredFollowingNoChatItems.length !== 0
						) {
						...
						if (following && following.length !== 0)
							allPublicUsersExcludeFolloiwing = usersCtx.users.filter(
								(user) =>
									user.public === 1 &&
									!following.some(
										(followingUser) => followingUser.id === user.id
						...
						if (
							allPublicUsersExcludeFolloiwing &&
							allPublicUsersExcludeFolloiwing.length !== 0
						) {
							filteredOtherNoChatItems = allPublicUsersExcludeFolloiwing.filter(
								(publicUser) => {
									if (!allChatItemArr) return false;
									return !allChatItemArr.some(
										(chatItem) => publicUser.id === chatItem.sourceid
									);
								}
							);
						}
						...
						console.log(
							"filteredOther Without ChatItems",
							filteredOtherNoChatItems
						);
						...
						if (
							filteredOtherNoChatItems &&
							filteredOtherNoChatItems.length !== 0 &&
							allOtherChatItems &&
							allOtherChatItems.length !== 0
						) {
							...
							filteredOtherNoChatItems &&
							filteredOtherNoChatItems.length !== 0 &&
							(!allOtherChatItems || allOtherChatItems.length === 0)
						) {
	...
	const requestToFollowHandler = (followUser) => {
		...
		if (wsCtx.websocket !== null)
			wsCtx.websocket.send(JSON.stringify(followPayloadObj));
	};
	...
	const followHandler = (followUser) => {
		if (following && following.length !== 0) {
			...
			setFollowingChat((prevFollowingChat) => [
				...new Set(prevFollowingChat),
				followUser,
			]);
	...
	const unfollowHandler = (unfollowUser) => {
		...
		setFollowing((prevFollowing) =>
			prevFollowing.filter(
				(followingUser) => followingUser.id !== unfollowUser.id
			)
		);
		setFollowingChat((prevFollowingChat) =>
			prevFollowingChat.filter(
				(followingChatUser) => followingChatUser.id !== unfollowUser.id
			)
		);
		const storedFollowing = JSON.parse(localStorage.getItem("following"));
		const curFollowing = storedFollowing.filter(
			(followingUser) => followingUser.id !== unfollowUser.id
		);
		localStorage.setItem("following", JSON.stringify(curFollowing));
		console.log(
			"locally stored following (unfol)",
			JSON.parse(localStorage.getItem("following"))
		);
	};
	...
	const receiveMsgHandler = (friendId, open, isFollowing) => {
		if (isFollowing) {
			const targetUser = following.find(
				(followingUser) => followingUser.id === +friendId
			);
			...
			if (!open) {
				...
				if (wsCtx.websocket !== null)
					wsCtx.websocket.send(JSON.stringify(privateChatNotiPayloadObj));
			}
			...
			// if one or both of the users is public and can chat coz of that
			...
			setOtherListedChatUsers((prevList) => [
				targetUser,
				...prevList.filter((otherChatUser) => otherChatUser.id !== +friendId),
			]);
			console.log("after add chat noti target user", targetUser);
		}
	};
