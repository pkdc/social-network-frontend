...
export const JoinedGroupContextProvider = (props) => {
	...
	const joinedGroupingUrl = `http://https://notfacebook-b2511391168d.herokuapp.com/group-member?userid=${selfId}`;
	const requestedGroupUrl = `http://https://notfacebook-b2511391168d.herokuapp.com/group-request-by-user?id=${selfId}`;
		...
		fetch(requestedGroupUrl)
			...
			.then((data) => {
				...
				localStorage.setItem(
					"requestedGroups",
					JSON.stringify(requestedGroupsArr)
				);
			})
			.catch((err) => console.log(err));
	};
		...
		(async function () {
			try {
				const promises = [
					...
					fetchGroupChatData(
						`http://https://notfacebook-b2511391168d.herokuapp.com/group-chat-item?userid=${selfId}`
					),
				];
				...
				if (!allGrpChatItemArr) {
					...
					const filteredGroupChatItems = allGrpChatItemArr.filter(
						(chatItem) => {
							...
							console.log(
								"returned",
								joinedGroupsArr.some(
									(grp) =>
										grp.id == chatItem.groupid && +selfId === chatItem.userid
								)
							);
							return joinedGroupsArr.some(
								(grp) =>
									grp.id === chatItem.groupid && +selfId === chatItem.userid
							);
						}
					...
					const groupChatItems = filteredGroupChatItems.map((chatItem) => {
						const matchedGroups = joinedGroupsArr.find(
							(grp) => grp.id === chatItem.groupid
						);
					...
					console.log(
						...
						filteredGroupsNoChatItems
					);
					const finalGroupChatItems = [
						...groupChatItems,
						...filteredGroupsNoChatItems,
					];
	...
	const requestToJoinHandler = (joinGrpId) => {
		...
		if (wsCtx.websocket !== null)
			wsCtx.websocket.send(JSON.stringify(joinGrpPayloadObj));
	};
	...
	const InviteToJoinHandler = (grpid, InvitedUserId) => {
		...
		if (wsCtx.websocket !== null)
			wsCtx.websocket.send(JSON.stringify(InviteToJoinPayloadObj));
	};
	...
	const joinHandler = (toJoinGrp, user) => {
		...
		console.log(
			"locally stored joined grp (jg ctx)",
			JSON.parse(localStorage.getItem("joined-grps"))
		);
	};
	...
	const receiveMsgHandler = (groupId, open) => {
		...
		if (!open) {
			...
			if (wsCtx.websocket !== null)
				wsCtx.websocket.send(JSON.stringify(groupChatNotiPayloadObj));
		}
		// move userId chat item to the top
		setJoinedGrps((prevJoinedGrps) => [
			targetGroup,
			...prevJoinedGrps.filter((joinedGrp) => joinedGrp.id !== +groupId),
		]);
