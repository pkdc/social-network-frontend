...
export const WebSocketContextProvider = (props) => {
	...
	useEffect(() => {
		const newSocket = new WebSocket(
			"ws://https://notfacebook-b2511391168d.herokuapp.com/ws"
		);
		...
		newSocket.onmessage = (e) => {
			...
			if (msgObj.label === "p-chat") {
				...
				if (
					...
					console.log(
						"ws receives noti follow reply type (wsctx): ",
						msgObj.type
					);
					console.log(
						"ws receives noti follow reply accepted (wsctx): ",
						msgObj.accepted
					);
					...
					console.log(
						"ws receives noti join-req-reply type (wsctx): ",
						msgObj.type
					);
					console.log(
						"ws receives noti join-req-reply accepted (wsctx): ",
						msgObj.accepted
					);
					...
					console.log(
						"ws receives noti invitation-reply type (wsctx): ",
						msgObj.type
					);
					console.log(
						"ws receives noti invitation-reply accepted (wsctx): ",
						msgObj.accepted
					);
				...
				console.log(
					"ws receives online-status onlineuserids (wsctx): ",
					msgObj.onlineuserids
				);
